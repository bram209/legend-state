...
"types.d.ts" was too generic of a name now that we have a lot of configuration options, so we are naming them more specifically in a "types" folder.
...
The reactive components are now better named and more easily customizable with configuration functions, exported from the normal `/react` path. See [Render an observableselector directly](../configuring/#enablereactcomponents-enablereactnativecomponents) for more details.
